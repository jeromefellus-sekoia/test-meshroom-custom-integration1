...
def pull_automation_library(path: Path):
    ...
    for manifest in path.rglob("manifest.json"):
        ...
        overwrite_directory(
            module, sekoia.path / "integrations" / product_name / "dist" / "automation"
        )
...
def get_automation_connector_by_uuid(
    repo: Path, module_uuid: str, uuid: str
) -> dict | None:
...
def pull_intake_formats(path: Path):
    ...
    for manifest in path.rglob("_meta/manifest.yml"):
        ...
        if automation_module_uuid and (
            automation_module := get_automation_module_by_uuid(
                path.parent, manifest_data["automation_module_uuid"]
            )
        ):
            with open(automation_module / "manifest.json", "r") as file:
                ...
                settings = ProductSetting.from_json_schema(
                    ...
                    force_secret={
                        r"intake_key|password|token|secret"
                    },  # Ensure intake_keys and password/token are stored as secrets
                )
        ...
        i.mode = mode
        i.documentation_url = (
            f"https://docs.sekoia.io/operation_center/integration_catalog/uuid/{uuid}"
        )
        ...
        if mode == "push":
            ...
            if not get_integration(product_name, "sekoia", "events", "producer", mode):
                ...
                dst.add_setup_step(
                    "Follow syslog forwarding instructions",
                    syslog_forwarding_instructions,
                )
                dst.save()
                info("✓ Created integration", dst)

        if mode == "pull":
            connector = get_automation_connector_by_uuid(
                path.parent, automation_module_uuid, automation_connector_uuid
            )
            connector_settings = [
                s
                for s in ProductSetting.from_json_schema(
                    ...
                    force_secret={
                        r"intake_key|password|token|secret"
                    },  # Ensure intake_keys and passwords/tokens are stored as secrets
                )
                if s.name
                not in (
                    "intake_key",
                    "intake_server",
                )  # intake_key and intake_server are automatically handled at intake creation
            ]
            ...
            if settings:
                i.settings.append(
                    ProductSetting(
                        name="module_configuration", type="object", properties=settings
                    )
                )
